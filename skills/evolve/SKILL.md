...
- **Never modify scripts/format_issues.py.** That sanitizes untrusted input.
- **Never modify scripts/build_site.py.** That builds the public site.
...
## Using docs.rs when making changes

**Before modifying your own source code, if you're working with unfamiliar APIs:**

1. **Always fetch the docs first** using the `fetch_docs` tool:
   ```json
   {"crate_name": "yoagent"}
   ```

2. **If you're not sure of the crate name**, use `search_docs`:
   ```json
   {"query": "http client"}
   ```

3. **Write what you learn to LEARNINGS.md** so future sessions don't search for the same thing.

4. **Never guess at API names.** If you're about to call a method you haven't used before, fetch the docs FIRST.

This is the same rule from the system prompt and research skill - it applies even when you're modifying yourself.
